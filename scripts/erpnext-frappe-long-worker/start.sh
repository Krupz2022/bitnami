...
if erpnext_is_frappe_worker_not_running "long"; then
    ...
    BITNAMI_QUIET=1 nohup /opt/bitnami/scripts/erpnext-frappe-long-worker/run.sh >>"$ERPNEXT_FRAPPE_WORKER_LOG_FILE" 2>&1 &
    if ! retry_while "erpnext_is_frappe_worker_running long"; then
        error "erpnext-frappe-long-worker did not start"
        error_code=1
    else
        info "erpnext-frappe-long-worker started"
    fi
else
