...
redis_log_file="${ERPNEXT_BASE_DIR}/frappe-bench/logs/redis-queue.log"
...
if is_redis_not_running "$redis_pid_file"; then
    ...
    BITNAMI_QUIET=1 nohup /opt/bitnami/scripts/erpnext-redis-queue/run.sh >>"$redis_log_file" 2>&1 &
    if ! retry_while "is_redis_running ${redis_pid_file}"; then
        error "erpnext-redis-queue did not start"
        error_code=1
    else
        info "erpnext-redis-queue started"
    fi
else
    info "erpnext-redis-queue is already running"
fi
